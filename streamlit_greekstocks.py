...
rs=backtest_portfolio(df,dataset=800,l_days=700,momentum_window=momentum_window,minimum_momentum=minimum_momentum,portfolio_size=portfolio_size,tr_period=5,cutoff=cutoff,port_value=port_value,a_v=0)
st.write(f'Με αρχικό κεφάλαιο {port_value}€, θα κάναμε {rs["trades"]} συναλλαγές ανά  5 ημέρες, θα είχαμε μια απόδοση {rs["tot_ret"]} και θα συγκεντρώναμε {rs["final port_value"]}€')
st.write('   ')
rs=backtest_portfolio(df,dataset=800,l_days=700,momentum_window=momentum_window,minimum_momentum=minimum_momentum,portfolio_size=portfolio_size,tr_period=10,cutoff=cutoff,port_value=port_value,a_v=0)
st.write(f'Με αρχικό κεφάλαιο {port_value}€, θα κάναμε {rs["trades"]} συναλλαγές ανά 10 ημέρες, θα είχαμε μια απόδοση {rs["tot_ret"]} και θα συγκεντρώναμε {rs["final port_value"]}€')
st.write('   ')
...
st.write(f'Με αρχικό κεφάλαιο {port_value}€, θα κάναμε {rs["trades"]} συναλλαγές ανά 20 ημέρες, θα είχαμε μια απόδοση {rs["tot_ret"]} και θα συγκεντρώναμε {rs["final port_value"]}€')
st.write('   ')
