...
port_value=st.sidebar.slider('Αρχική επένδυση στο χαρτοφυλάκιο €', 1000, 10000, 3000,500)
...
for metoxi in stocks:
    ...
    try:
        sma = [50, 150, 200]
        for x in sma:
            df1["SMA_"+str(x)] = round(df_t.iloc[:,4].rolling(window=x).mean(), 2)

        currentClose = df_t[metoxi][-1]
        moving_average_50 = df1["SMA_50"][-1]
        moving_average_150 = df1["SMA_150"][-1]
        moving_average_200 = df1["SMA_200"][-1]
        low_of_52week = min(df_t[metoxi][-260:])
        high_of_52week = max(df_t[metoxi][-260:])
        
        try:
            moving_average_200_20 = df1["SMA_200"][-20]
