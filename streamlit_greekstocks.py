...
st.write(
    'Στον παρακάτω πίνακα φαίνεται η λίστα των μετοχών με τις ημερήσιες τιμές κλεισίματός τους για τις τελευταίες ' + str(
        l_close_min) + ' μέρες')
st.write(
    'Οι μετοχές που έχουν αρχικά επιλεγεί είναι οι παρακάτω που βλέπουμε στον πίνακα των τιμών κλεισίματος τους. Τα σύμβολά τους είναι τα ονόματα των στηλών του πίνακα.')
st.write(
...
st.write(
    f'Συσσωρευτικές αποδόσεις όλων των παραπάνω μετοχών για τις {l_close_min} ημέρες.')
df_cum_ret = 100 * (df_cum_ret.iloc[-1:, :] - 1)
st.dataframe(df_cum_ret)
st.write('Πίνακας των ημερησίων ποσοστιαίων μεταβολών όλων των Μετοχών για τις 10 τελευταίες ημέρες')
...
st.write('Στον παρακάτω πίνακα βλέπουμε τις τελευταίες ημερήσιες τιμές των μετοχών που έχουν επιλεγεί για το Χαρτοφυλάκιό μας')
st.dataframe(df_t.tail())
st.dataframe(df_buy)
st.write(
    'Στον παραπάνω πίνακα με το Χαρτοφυλάκιο που έχει δημιουργηθεί βλέπουμε το σύμβολο της κάθε μετοχής, στην στήλη "weights" το ποσοστό συμμετοχής της στο χαρτοφυλάκιο,')
...
κάθε 5 (εβδομάδα) ,10 (15ήμερο),20 (μήνα) ημέρες στις τελευταίες Υ ημέρες όπως έχουμε επιλέξει για το backtest.**''')
...
st.subheader(
    'Εάν θέλεις μπορείς να "σώσεις" το παραπάνω χαρτοφυλάκιο κατεβάζοντάς το στον υπολογιστή σου. Απλά δώσε ένα όνομα και πάτησε το κουμπί για να σου αποσταλεί σαν αρχείο.')
...
if st.button('Σώσε αυτό το Χαρτοφυλάκιο', key=1):
    ...
    download_button_str = download_button(
        df_buy, filename, f'Πάτα εδώ για να κατεβάσεις το αρχείο {filename}', pickle_it=False)
...
file = st.file_uploader(
    "Σύρτε και αφήστε εδώ το Χαρτοφυλάκιό που έφτιαξε η εφαρμογή για σας (*.csv)", type='csv')
show_file = st.empty()
if not file:
    show_file.info("")
else:
